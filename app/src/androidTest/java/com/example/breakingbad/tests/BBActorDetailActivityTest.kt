...
import com.example.breakingbad.setup.mockvm.MockBBActorDetailVmProvider
...
class BBActorDetailActivityTest : InstrumentedTestSetup() {

    @get:Rule
    val testRule: ActivityTestRule<BBActorDetailActivity> =
        ActivityTestRule(
            BBActorDetailActivity::class.java,
            false, false
        )

    private lateinit var mockViewModel: BBActorDetailViewModel
    private lateinit var mockItem: BBActor

    @Before
    fun setup() {
        mockViewModel = MockBBActorDetailVmProvider.provideMockBBActorDetailVm()
    ...
    fun liveDataChanges_then_uiUpdatedWithExpectedData() {
        // given
        every { mockViewModel.model } returns MockBBActorDetailVmProvider.model
    ...
    private fun launchActivityAndMockLiveData() {
        ...
        testRule.activity.runOnUiThread {
            MockBBActorDetailVmProvider.mModel.value = mockItem
        }
    }
